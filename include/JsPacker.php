...
class JavaScriptPacker {
	...
	public function __construct($_script, $_encoding = 95, $_fastDecode = false, $_specialChars = false) {
	...
	private function _bootStrap($packed, $keywords) {
		...
		if ($this->_fastDecode) {
			...
			elseif ($ascii < 36) $decode = preg_replace($ENCODE, $inline, $decode);
	...
	private function _escape95Bis($match) {
		if (is_array($match)) {
			return '\x' . ((string) dechex(ord($match[0])));
		} else {
