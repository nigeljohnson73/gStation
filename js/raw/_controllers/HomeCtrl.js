		...
		.controller(
				...
				[
						...
						function($scope, $interval, apiSvc) {
							...
							var getEnv = function() {
								apiSvc.call("getEnv", {}, function(data) {
									...
									if (data.success) {
										...
										angular.forEach($scope.env.sensors,
												function(item, index) {
													addSensorReading(
															$scope.temps, item,
															item.temperature);
										...
										addSensorReading($scope.temps, obj,
												obj.temperature);
										addSensorReading($scope.humds, obj,
												obj.humidity);
										updateGraph('#temperature-graph',
												$scope.temps,
												"Temperature Readings");
										updateGraph('#humidity-graph',
												$scope.humds,
												"Humidity Readings");
									} else {
										$scope.env = null;
							...
							var getSnapshotImage = function() {
								apiSvc
										.call(
												...
												function(data) {
													logger(
															"HomeCtrl::handleGetSnapshotImage()",
															"dbg");
													logger(data, "dbg");
													if (data.success) {
							...
							getSnapshotImage();
							$scope.snapshot_image_api_call = $interval(
									getSnapshotImage, 10000);
